...
namespace teal::parser
{
    ...
    enum class TokenType {
        ...
        NIL, TRUE, FALSE,
        FUNCTION, END, DO, IF, THEN, ELSE, ELSEIF,
        WHILE, REPEAT, UNTIL, FOR, IN,
        BREAK, GOTO, RETURN,
        LOCAL,

        //TEAL KEYWORDS//
        GLOBAL, RECORD, INTERFACE, ENUM, TYPE, WHERE,
        AS, IS, MACROEXP,
        //---//

        AND, OR, NOT,
        ASSIGN, EQUALS, NOT_EQ, LESS, LESS_EQ, GREATER, GREATER_EQ,
        CONCAT, ADD, SUB, MUL, DIV, FLOOR_DIV, MOD, POW,
    ...
    struct Token {
        ...
        constexpr bool is_null_token() const
        ...
        constexpr inline bool is_teal_keyword() const
        {
            return type_is_teal_keyword(type);
        }

        std::optional<Token> teal_to_name() const
        {
            const auto mkname = [this](std::string_view id) {
                return Token { TokenType::NAME, String(id), line, column };
            };
            switch (type) {
            case TokenType::TYPE: return mkname("type");
            case TokenType::AS: return mkname("as");
            case TokenType::IS: return mkname("is");
            case TokenType::WHERE: return mkname("where");
            case TokenType::GLOBAL: return mkname("global");
            case TokenType::RECORD: return mkname("record");
            case TokenType::INTERFACE: return mkname("interface");
            case TokenType::ENUM: return mkname("enum");
            case TokenType::MACROEXP: return mkname("macroexp");
            default:
                return std::nullopt;
            }
        }

        //Debug stuff can just use regular allocator
        static constexpr std::string type_to_string(TokenType type)
        {
            ...
            case TokenType::MACROEXP: return "macroexp";
            ...
            case TokenType::SHIFT_L: return "<<";
            case TokenType::SHIFT_R: return ">>";
            case TokenType::LENGTH: return "#";
            case TokenType::L_PAREN: return "(";
            case TokenType::R_PAREN: return ")";
            case TokenType::L_BRACE: return "{";
            case TokenType::R_BRACE: return "}";
        ...
        {
            switch (type) {
            case TokenType::NAME: return std::format("Name({})", text);
            case TokenType::NUMBER: return std::format("Number({})", text);
            case TokenType::STRING: return std::format("String(\"{}\")", text);
            default: return type_to_string(type);
            }
        }
    };
    ...
    public:
        ...
        > {
            ...
            constexpr inline std::string to_string() const {
                ...
                case 6: return std::format("Too many errors ({})", std::get<TooManyErrors>(kind).error_count);
                default: return "Unknown";
                }
            }
        };

        Lexer(std::string_view source, Allocator alloc = Allocator(std::pmr::get_default_resource())):
            max_errors(10),
            allocator(alloc),
            src(String(source, allocator)),
            length(source.size()),
            pos(0), line(1), col(1),
            tokens(allocator), errors(allocator)
        {}
        std::pair<Vector<Token>, Vector<Error>> tokenize();
    ...
    private:
        ...
        constexpr inline void push_error(Error::Kind_t err)
        {
            errors.push_back(make_error(err));
        }
        ...
        bool skip_comment() {
            ...
            if (peek() == '[') {
                consume();
                int eq_count = 0;
                while (peek() == '=') { consume(); eq_count++; }
                if (peek() == '[') {
                    consume();
                    auto end_marker = string("]");
        ...
        Token read_string() {
            ...
            if (not closed) {
                push_error(UnterminatedStringLiteral());
            }
            return { TokenType::STRING, str, start_line, start_col };
        }
        ...
        bool match_end_marker(std::string_view end_marker) {
            if (pos + end_marker.size() > length) {
                return false;
            }
            for (size_t i = 0; i < end_marker.size(); i++) {
                if (src[pos + i] != end_marker[i]) {
                    return false;
                }
            }
            for (size_t i = 0; i < end_marker.size(); i++) {
        ...
        Token read_name() {
            ...
            static const std::unordered_map<std::string_view, TokenType> keywords = {
                ...
                {"function", TokenType::FUNCTION}, {"end", TokenType::END}, {"do", TokenType::DO}, {"if", TokenType::IF},
                {"then", TokenType::THEN}, {"else", TokenType::ELSE}, {"elseif", TokenType::ELSEIF}, {"while", TokenType::WHILE},
                {"repeat", TokenType::REPEAT}, {"until", TokenType::UNTIL}, {"for", TokenType::FOR}, {"in", TokenType::IN},
                {"break", TokenType::BREAK}, {"goto", TokenType::GOTO}, {"return", TokenType::RETURN},
                {"local", TokenType::LOCAL}, {"global", TokenType::GLOBAL}, {"record", TokenType::RECORD},
                {"interface", TokenType::INTERFACE}, {"enum", TokenType::ENUM}, {"type", TokenType::TYPE},
                {"where", TokenType::WHERE},
                {"and", TokenType::AND}, {"or", TokenType::OR}, {"not", TokenType::NOT},
                {"as", TokenType::AS}, {"is", TokenType::IS},
                {"macroexp", TokenType::MACROEXP}
            };
            auto it = keywords.find(name);
            if (it != keywords.end()) {
                return { it->second, name, start_line, start_col };
            }
