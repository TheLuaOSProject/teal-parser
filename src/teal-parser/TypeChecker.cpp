        ...
        if (gp.is.has_value()) {
            ...
            if (not constraintSym) {
                ...
                if (constraintSym->kind != TypeSymbol::Kind::Interface) {
                    errors.addError(node->line, node->column, "Generic constraint must be an interface type");
                }
            }
        }
...
void TypeChecker::declareType(EnumDeclarationStatement *node, std::shared_ptr<TypeSymbol> parent) {
    ...
    if (node->body) {
        for (auto &elem : node->body->elements) {
            typeSym->enum_values.push_back(elem);
        }
    }
...
void TypeChecker::defineType(std::shared_ptr<TypeSymbol> typeSymbol) {
    ...
    if (typeSymbol->kind == TypeSymbol::Kind::Record or typeSymbol->kind == TypeSymbol::Kind::Interface) {
        ...
        for (auto &[key, val] : typeSymbol->nested_types) {
            defineType(val);
            if (val->type) {
                typeSymbol->fields[key] = val->type;
            }
        }
...
TypePtr TypeChecker::resolveTypeNode(TypeNode *typeNode, std::shared_ptr<TypeSymbol> currentTypeSym) {
    ...
    if (auto basic = dynamic_cast<BasicTypeNode*>(typeNode)) {
        ...
        if (currentTypeSym) {
            for (auto &tv : currentTypeSym->type_params) {
                if (tv->kind == Type::Kind::TypeVar and tv->type_var->name == name) {
                    return tv;
                }
            }
        }
    ...
    if (auto nom = dynamic_cast<NominalTypeNode*>(typeNode)) {
        ...
        for (auto &argNode : nom->type_arguments) {
            typeArgs.push_back(resolveTypeNode(argNode.get(), currentTypeSym));
        }
        if (not sym->type_params.empty() and typeArgs.size() != sym->type_params.size()) {
            errors.addError(typeNode->line, typeNode->column, "Generic type parameters count mismatch for " + sym->name);
        }
        if (sym->kind == TypeSymbol::Kind::Record or sym->kind == TypeSymbol::Kind::Interface) {
            ...
            if (not typeArgs.empty()) {
                errors.addError(typeNode->line, typeNode->column, "Enum type cannot have type parameters");
            }
            return sym->type;
        } else if (sym->kind == TypeSymbol::Kind::Alias) {
            if (not sym->alias_target) {
                defineType(sym);
            }
            if (sym->alias_target) {
                return sym->alias_target;
            }
            return Type::makeAny();
        }
        return sym->type ? sym->type : Type::makeAny();
    ...
    if (auto table = dynamic_cast<TableTypeNode*>(typeNode)) {
        if (table->is_map) {
            ...
            if (table->element_types.size() > 1) {
                ...
                for (auto &elemNode : table->element_types) {
                    elems.push_back(resolveTypeNode(elemNode.get(), currentTypeSym));
                }
    ...
    if (auto funcNode = dynamic_cast<FunctionTypeNode*>(typeNode)) {
        ...
        for (auto &rnode : funcNode->return_types) {
            rets.push_back(resolveTypeNode(rnode.get(), currentTypeSym));
        }
    ...
    if (auto unionNode = dynamic_cast<UnionTypeNode*>(typeNode)) {
        ...
        for (auto &optNode : unionNode->options) {
            options.push_back(resolveTypeNode(optNode.get(), currentTypeSym));
        }
...
std::unordered_map<std::string, TypeChecker::NarrowInfo> TypeChecker::analyzeCondition(Expression *cond) {
    ...
    if (auto isExp = dynamic_cast<IsTypeExpression*>(cond)) {
                ...
                if (targetType) {
                    ...
                    if (varType->kind == Type::Kind::Union) {
                        ...
                        for (auto &opt : varType->union_members) {
                            if (opt->isAssignableTo(targetType) and targetType->isAssignableTo(opt)) {
                                inter.push_back(opt);
                            }
                        }
                        if (not inter.empty()) {
                            thenType = inter.size() == 1 ? inter[0] : Type::makeUnion(inter);
                        }
                    }
                    TypePtr elseType;
                    if (varType->kind == Type::Kind::Union) {
                        ...
                        for (auto &opt : varType->union_members) {
                            if (not opt->isAssignableTo(targetType)) {
        ...
        if ((equality or inequality) and
            ((dynamic_cast<NilExpression*>(left) and dynamic_cast<NameExpression*>(right)) or
             (dynamic_cast<NameExpression*>(left) and dynamic_cast<NilExpression*>(right)))) {
                ...
                if (var) {
                    ...
                    if (varType->kind == Type::Kind::Union) {
                        ...
                        for (auto &opt : varType->union_members) {
                            if (opt->kind == Type::Kind::Nil or opt->kind == Type::Kind::Boolean) {
                                falsy.push_back(opt->kind == Type::Kind::Boolean ? Type::makeBoolean() : opt);
                            }
                        }
                    ...
                    if (elseType->kind == Type::Kind::Union) {
                        ...
                        for (auto &opt : elseType->union_members) {
                            if (opt->kind != Type::Kind::Nil and opt->kind != Type::Kind::Boolean) {
                                truthy.push_back(opt);
                            }
                        }
...
void TypeChecker::checkIf(IfStatement *node) {
    for (size_t i = 0; i < node->if_branches.size(); ++i) {
        ...
        symbols.popScope();
        for (auto &pr : savedTypes) {
            pr.first->type = pr.second;
        }
    }
    if (node->else_block) {
        symbols.pushScope();
...
void TypeChecker::checkForNumeric(ForNumericStatement *node) {
    ...
    TypePtr loopVarType;
    if ((startType == nullptr or startType->kind == Type::Kind::Integer) and
        (endType == nullptr or endType->kind == Type::Kind::Integer) and
        (stepType->kind == Type::Kind::Integer)) {
...
void TypeChecker::checkForIn(ForInStatement *node) {
    ...
    for (auto &expr : node->exprs) {
        exprTypes.push_back(checkExpression(expr.get()));
    }
    ...
    while (nameTypes.size() < node->names.size()) {
        nameTypes.push_back(Type::makeAny());
    }
    symbols.pushScope();
    for (size_t i = 0; i < node->names.size(); ++i) {
        symbols.define(node->names[i], nameTypes[i], false, false);
    }
    loopDepth++;
    checkStatement(node->body.get());
    loopDepth--;
...
void TypeChecker::checkFunctionDecl(FunctionDeclarationStatement *node) {
    ...
    for (auto &retNode : node->body->return_types) {
        returnTypes.push_back(resolveTypeNode(retNode.get()));
    }
            ...
            if (classSym) {
                ...
                if (fields.find(node->method_name) != fields.end()) {
                    ...
                    if (not funcType->equals(existingType)) {
                        errors.addError(node->line, node->column, "Method signature does not match previously declared type for " + node->method_name);
                    }
                }
        ...
        if (not node->name_path.empty()) {
            ...
            if (not baseVar) {
                ...
                if (baseType->kind == Type::Kind::Record) {
                    ...
                    if (baseType->value->kind == Type::Kind::Any) {
                        baseType->value = funcType;
                    }
                }
            }
        } else {
            ...
            if (not ok) {
                ...
                if (existing and not funcType->equals(existing->type)) {
                    errors.addError(node->line, node->column, "Function redefinition with different type for " + funcName);
                }
            }
        }
    }
    symbols.pushScope();
    if (node->is_method) {
        symbols.define("self", paramTypes[0], false, false);
    }
    ...
    if (node->body->body) {
        checkStatement(node->body->body.get());
    }
    symbols.popScope();
}
...
void TypeChecker::checkVarDecl(VariableDeclarationStatement *node) {
    ...
    for (auto &val : node->values) {
        valueTypes.push_back(checkExpression(val.get()));
    }
    for (size_t i = valueTypes.size(); i < nVars; ++i) {
        valueTypes.push_back(Type::makeNil());
    }
    for (size_t i = 0; i < nVars; ++i) {
        ...
        if (initType and declaredType and not initType->isAssignableTo(declaredType)) {
            errors.addError(node->line, node->column, "Type mismatch in initialization of " + varName + ": cannot assign " + initType->toString() + " to " + declaredType->toString());
        }
        bool ok = symbols.define(varName, varType, is_const, node->visibility == Visibility::GLOBAL, attr);
        if (not ok) {
            errors.addError(node->line, node->column, "Duplicate definition of variable " + varName);
        }
                ...
                if (auto tableLit = dynamic_cast<TableConstructorExpression*>(node->values[i].get())) {
                    if (varType->key->kind == Type::Kind::Enum) {
                        ...
                        for (auto &enumVal : enumSym->enum_values) {
                            ...
                            if (not found) {
                                errors.addError(node->line, node->column, "Table not total: missing key \"" + enumVal + "\"");
                            }
                        }
                        ...
                        if (not hasTrue or not hasFalse) {
                            errors.addError(node->line, node->column, "Table not total: missing key " + std::string(not hasTrue ? "\"true\"" : "\"false\""));
                        }
                    }
                }
            }
...
void TypeChecker::checkAssignment(AssignmentStatement *node) {
    ...
    for (auto &expr : node->right) {
        rightTypes.push_back(checkExpression(expr.get()));
    }
    if (rightTypes.size() < node->left.size()) {
        while (rightTypes.size() < node->left.size()) {
            rightTypes.push_back(Type::makeNil());
        }
    }
    for (size_t i = 0; i < node->left.size(); ++i) {
        ...
        if (auto nameExp = dynamic_cast<NameExpression*>(lhs)) {
            ...
            if (var->is_const) {
                errors.addError(lhs->line, lhs->column, "Cannot assign to constant variable " + name);
            }
            if (not rhsType->isAssignableTo(var->type)) {
                errors.addError(lhs->line, lhs->column, "Type mismatch: cannot assign " + rhsType->toString() + " to " + var->type->toString());
            }
            ...
            if (tableType->kind == Type::Kind::Array) {
                if (indexType->kind != Type::Kind::Number and indexType->kind != Type::Kind::Integer) {
                    errors.addError(lhs->line, lhs->column, "Array index is not a number");
                }
                if (not rhsType->isAssignableTo(tableType->element)) {
                    errors.addError(lhs->line, lhs->column, "Type mismatch in array assignment: expected " + tableType->element->toString());
                }
            } else if (tableType->kind == Type::Kind::Map) {
                if (not indexType->isAssignableTo(tableType->key)) {
                    errors.addError(lhs->line, lhs->column, "Map index type mismatch: cannot index " + tableType->toString() + " with " + indexType->toString());
                }
                if (not rhsType->isAssignableTo(tableType->value)) {
                    errors.addError(lhs->line, lhs->column, "Type mismatch in map assignment: expected " + tableType->value->toString());
                }
            } else if (tableType->kind == Type::Kind::Tuple) {
                if (indexType->kind != Type::Kind::Number and indexType->kind != Type::Kind::Integer) {
                    errors.addError(lhs->line, lhs->column, "Tuple index is not a number");
                }
            } else if (tableType->kind == Type::Kind::Record) {
                if (tableType->record->array_element_type) {
                    if (indexType->kind != Type::Kind::Number and indexType->kind != Type::Kind::Integer) {
                        errors.addError(lhs->line, lhs->column, "Array part index is not a number");
                    }
                    if (not rhsType->isAssignableTo(tableType->record->array_element_type)) {
                        errors.addError(lhs->line, lhs->column, "Type mismatch in array part assignment: expected " + tableType->record->array_element_type->toString());
                    }
                } else {
            ...
            if (objType->kind == Type::Kind::Record) {
                ...
                if (it != fields.end()) {
                    ...
                    if (not rhsType->isAssignableTo(fieldType)) {
                        errors.addError(lhs->line, lhs->column, "Type mismatch for field '" + fieldName + "': cannot assign " + rhsType->toString() + " to " + fieldType->toString());
                    }
                } else {
                ...
                if (objType->value->kind != Type::Kind::Any and not rhsType->isAssignableTo(objType->value)) {
                    errors.addError(lhs->line, lhs->column, "Type mismatch for table field '" + fieldName + "'");
                }
                if (objType->value->kind == Type::Kind::Any) {
                    objType->value = rhsType;
                }
...
TypePtr TypeChecker::checkExpression(Expression *expr) {
...
TypePtr TypeChecker::checkExpression(Expression *expr, const TypePtr &expectedType) {
    ...
    if (auto num = dynamic_cast<NumberExpression*>(expr)) {
        ...
        for (char c : num->value) {
            if (c == '.' or c == 'e' or c == 'E') { isInt = false; break; }
        }
    ...
    if (dynamic_cast<BooleanExpression*>(expr)) {
        return Type::makeBoolean();
    }
    if (dynamic_cast<NilExpression*>(expr)) {
        return Type::makeNil();
    }
    if (auto nameExp = dynamic_cast<NameExpression*>(expr)) {
        ...
        if (baseType->kind == Type::Kind::Function) {
            ...
            if (provided < minRequired or (not sig->param_varargs and provided > sig->param_types.size())) {
                ...
                for (size_t i = 0; i < checkCount; ++i) {
                    if (not argTypes[i]->isAssignableTo(sig->param_types[i])) {
                        errors.addError(expr->line, expr->column, "Argument " + std::to_string(i+1) + " type mismatch: expected " + sig->param_types[i]->toString() + ", got " + argTypes[i]->toString());
                    }
                }
            }
    ...
    if (auto binOp = dynamic_cast<BinaryOperationExpression*>(expr)) {
        ...
        case TokenType::MOD:
        case TokenType::POW:
            if (not ltype->isAssignableTo(Type::makeNumber()) or not rtype->isAssignableTo(Type::makeNumber())) {
                errors.addError(expr->line, expr->column, "Arithmetic operator applied to non-numeric type");
            }
        ...
        case TokenType::CONCAT:
            if (not ltype->isAssignableTo(Type::makeString()) and not ltype->isAssignableTo(Type::makeNumber())) {
                errors.addError(expr->line, expr->column, "Concat left operand not string or number");
            }
            if (not rtype->isAssignableTo(Type::makeString()) and not rtype->isAssignableTo(Type::makeNumber())) {
                errors.addError(expr->line, expr->column, "Concat right operand not string or number");
            }
            return Type::makeString();
        ...
        case TokenType::GREATER_EQ:
            if (not ltype->equals(rtype) and ltype->kind != Type::Kind::Any and rtype->kind != Type::Kind::Any) {
                errors.addError(expr->line, expr->column, "Comparing incompatible types " + ltype->toString() + " and " + rtype->toString());
            }
            return Type::makeBoolean();
        case TokenType::AND: {
            TypePtr resultType;
    ...
    if (auto unOp = dynamic_cast<UnaryOperationExpression*>(expr)) {
        ...
        case TokenType::SUB:
            if (not operandType->isAssignableTo(Type::makeNumber())) {
                errors.addError(expr->line, expr->column, "Unary minus on non-numeric type");
            }
    ...
    if (auto tableLit = dynamic_cast<TableConstructorExpression*>(expr)) {
        if (expectedType) {
            if (expectedType->kind == Type::Kind::Array or expectedType->kind == Type::Kind::Tuple) {
                ...
                if (expectedType->kind == Type::Kind::Tuple) {
                    tupleTypes = expectedType->tuple_types;
                }
                size_t index = 0;
                for (auto &field : tableLit->fields) {
                    TypePtr valType;
                    if (std::holds_alternative<std::unique_ptr<Expression>>(field)) {
                        ...
                        if (expectedType->kind == Type::Kind::Array) {
                            if (not valType->isAssignableTo(elemType)) {
                                errors.addError(expr->line, expr->column, "Array element type mismatch: expected " + elemType->toString());
                            }
                        } else if (expectedType->kind == Type::Kind::Tuple) {
                            if (index < tupleTypes.size() and not valType->isAssignableTo(tupleTypes[index])) {
                                errors.addError(expr->line, expr->column, "Tuple element " + std::to_string(index+1) + " type mismatch");
                            }
                        }
                    } else {
                        const auto &kv = std::get<TableConstructorExpression::KeyValuePair>(field);
                        ...
                        if (expectedType->kind == Type::Kind::Map) {
                            if (not keyType->isAssignableTo(expectedType->key)) {
                                errors.addError(expr->line, expr->column, "Table key type mismatch: expected " + expectedType->key->toString());
                            }
                            if (not valueType->isAssignableTo(expectedType->value)) {
                                errors.addError(expr->line, expr->column, "Table value type mismatch: expected " + expectedType->value->toString());
                            }
                        } else {
                ...
                for (auto &field : tableLit->fields) {
                    if (std::holds_alternative<std::unique_ptr<Expression>>(field)) {
                        ...
                        if (not kType->isAssignableTo(keyType)) {
                            errors.addError(expr->line, expr->column, "Table key type mismatch");
                        }
                        if (not vType->isAssignableTo(valType)) {
                            errors.addError(expr->line, expr->column, "Table value type mismatch");
                        }
                    }
                }
                return expectedType;
        ...
        for (auto &field : tableLit->fields) {
            if (std::holds_alternative<std::unique_ptr<Expression>>(field)) {
                if (hasKV) {
                    errors.addError(expr->line, expr->column, "Mixed array and map literal entries");
                }
                ...
                if (not mapKeyType) {
                    ...
                    if (not kType->equals(mapKeyType)) {
                        mapKeyType = Type::makeUnion({ mapKeyType, kType });
                    }
                    if (not vType->equals(mapValType)) {
                        mapValType = Type::makeUnion({ mapValType, vType });
                    }
                }
            }
        }
        if (not hasKV) {
            if (not arrayElemType) {
                return Type::makeArray(Type::makeAny());
            }
            ...
            if (not mapKeyType) {
                return Type::makeMap(Type::makeAny(), Type::makeAny());
            }
...
void TypeChecker::checkStatement(Statement *stmt) {
    ...
    if (auto block = dynamic_cast<Block *>(stmt)) {
        ...
        for (auto &val : ret->values) {
            retTypes.push_back(checkExpression(val.get()));
        }
        // Compare retTypes with current function's return types if we had context
    }
    else if (dynamic_cast<BreakStatement*>(stmt)) {
        if (loopDepth <= 0) {
            errors.addError(stmt->line, stmt->column, "break statement not within a loop");
        }
    }
        ...
        if (callStmt->call) {
            checkExpression(callStmt->call.get());
        }
    }
}
