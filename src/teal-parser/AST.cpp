...
//                else v = std::get<Pointer<ExpressionNode>>(key)->serialise();
//                std::move(v);
//            }));
//     $field(value);
//     $field(type);
...
//                for (auto &v : fields) {
//                    if (std::holds_alternative<Pointer<ExpressionNode>>(v)) {
//                        arr.push_back(Value::from(std::get<Pointer<ExpressionNode>>(v)->serialise()));
//                    } else {
